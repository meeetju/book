...
There may be also a cluster of zookeepers (ensemble).
...
- Timestamp
- Offset number (unique across partitions)
...
so different files are used to store messages for the topic. Partitions also increase fault tolerance.
...
- Sends copies of each message to all Followers
...
Now we can send messages to the topic. We use the `broker-list` as we may provide more than one `kafka` broker addresses.

### Consume messages

Usually every consumer is connected to only one topic, but it is possible to connect to multiple topics.
Consumer may consumer from all or from one partition. Consumer may read from beginning (offset 0),
